...
parameter Real default_dose = 10;                 // default meal dose
...
parameter Real Gtb = if (Gpb <= Ke2) then ((Fsnc - EGPb + k1 * Gpb) / k2) else ((Fsnc - EGPb + Ke1 * (Gpb - Ke2)) / Vg + k1 * Gpb) / k2;
parameter Real Vm0 = if (Gpb <= Ke2) then ((EGPb - Fsnc)*(Km0 + Gtb) / Gtb) else ((EGPb - Fsnc - Ke1 * (Gpb-Ke2)) * (Km0+Gtb) / Gtb);
...
parameter Real HEb = max((SRb - m4*Ipb) / (SRb + m2*Ipb), 0);
...
//////////////////////////////////////////////////////////////////////////////
equation
meal_dose = default_dose; // eventually add Markov chain for meal variance simulation (one does not eat little for a long time...)
...
Qsto = Qsto1 + Qsto2;
der(Qsto1) = -Kmax*Qsto1 + meal_dose * meal;
